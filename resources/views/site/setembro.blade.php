     	...
     	@for($dia = 1, $itemAnterior = null, $finalSemana = array(1, 4, 5, 11, 12, 19, 20, 25, 26); $dia <= 31; $dia++)
			...
			@else
				...
				@for($horario = 9; $horario <= 11; $horario++)
                    @for($qtd = 1, $itemAtual = null, $achou = false; $qtd <= 2; $qtd++)
                        ...
                        {{$achou = false}}
                            ...
                            {!! csrf_field() !!}
                                <tr id='2019-05-{{$dia}} {{$horario}}:00'>
                                    ...
                                    <td class='id_td'>
                                        <input disabled type='text' id='id_monitorado' name='id_monitorado' class='id_mon' value="{{$itemAtual->monitorado->id_monitorado}}"/>
                                    </td>
                                    <td class='nome_td'>
                                        <input disabled class='nome_input' id="nome_monitorado" name='nome_monitorado' type="text" value="{{$itemAtual->monitorado->nome}}">
                                    </td>
                                    <td class='motivo_td'>
                                        <select disabled class='achou motivo_agendamento' name='motivo'>
                                            <option value='{{$itemAtual->agendamento->motivo}}'>{{$item->agendamento->motivo}}</option>
                                        </select>
                                    </td>
                                    <td class='materiais_td'>
                                        <input type='text' class='materiais'>
                                    </td>
                                    <td class='materiais_td'>
                                        <input type='text' class='materiais'>
                                    </td>
                                    ...
                                    <td class='compareceu_td'>
                                        <select class='compareceu' name='compareceu'>
                                            ...
                                            <option value='comp_sim'>Sim</option>
                                            <option value='comp_nao'>Não</option>
                                            <option value='remarcou'>Reagendou</option>
                                        </select>
                                    </td>
                                    <td class='acao_agend'>
                                        <a disabled  href='#' name='{{$dia}}{{$horario}}add' class='position action add' onClick="document.getElementById('mail_form{{$dia}}{{$horario}}{{$qtd}}').submit();">
                                            <span class='glyphicon glyphicon-plus'></span>
                                        </a>
                                       
                                        <a href='' name='{{$dia}}{{$horario}}delete' class='position action delete'>
                                            <span class='glyphicon glyphicon-trash'></span>
                                        </a>

                                        <a href='' name='{{$dia}}{{$horario}}edit' class='position action edit'>
                        ...
                        @else
                            ...
                            {!! csrf_field() !!}
                                <tr id='2019-05-{{$dia}} {{$horario}}:00'>
                                    ...
                                    <td class='acao_agend'>
                                        <a href='#' name='{{$dia}}{{$horario}}add' class='position action add' onClick="document.getElementById('mail_form{{$dia}}{{$horario}}{{$qtd}}').submit();">
                                            <span class='glyphicon glyphicon-plus'></span>
                                        </a>
                        ...
                        @if($achou)
                            ...
                            {!! csrf_field() !!}
                                <tr id='2019-05-{{$dia}} {{$horario}}:00'>
                                    ...
                                    <td class='id_td'>
                                        <input type='text' id='id_monitorado' name='id_monitorado' class='id_mon' value="{{$itemAtual->monitorado->id_monitorado}}"/>
                                    </td>
                                    <td class='nome_td'>
                                        <input class='nome_input' id="nome_monitorado" name='nome_monitorado' type="text" value="{{$itemAtual->monitorado->nome}}">
                                    </td>
                                    <td class='motivo_td'>
                                        <select class='motivo_agendamento' name='motivo'>
                                            <option value='{{$item->agendamento->motivo}}'>{{$item->agendamento->motivo}}</option>
                                        </select>
                                    </td>
                                    <td class='materiais_td'>
                                        <input type='text' class='materiais'>
